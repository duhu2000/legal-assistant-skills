...
description: |
  法律风险结构化分析与可视化 SKILL · 法律风险表达层。
  基于法律分析文本（如裁判文书、案件分析报告、合规审计意见、风险评估底稿等），执行"五步风险抽取模型"
  → 自动生成"四层可视化输出"，将线性的法律推理文本转化为网络化风险结构，支持内部决策与系统化风险管理。

  核心能力：
  - **五步风险抽取模型**：风险源识别 → 法律判断节点抽取 → 风险后果归类 → 商业影响推演 → 影响路径构建
  - **四层可视化输出**：① 风险雷达图（matplotlib，多维度评分）/ ② 风险矩阵（发生可能性 × 影响严重性 × 可干预度 × 可补救度）/ ③ 影响路径图（Mermaid，事实 → 判断 → 后果 → 影响四类节点）/ ④ 决策树（Mermaid，建议处置路径）
  - **唯一数据源头**：第三层影响路径图为唯一数据源，其他三层从中派生，确保数据一致性
  - **术语规范化**：风险指数（NRS）/ 传导风险值（PCR）/ 发生可能性（P）/ 影响严重性（I）/ 可干预程度（C）/ 可补救程度（R）—— 主报告中文，附录可缩写
  - **节点颜色编码**：🔴 风险源头 / 🔵 中间环节 / 🟠 风险后果 / 🟣 业务影响

  适用场景：法律分析报告 → 风险可视化 / 合规审计意见 → 决策树 / 案件分析 → 影响路径图 / 内部决策汇报材料。
  
  **注**：本 SKILL 是"风险表达层"工具，专注于将文本风险转化为可视化输出。如需"诉讼风险评估"原始数据收集与扫描，请配合使用 litigation-analysis-qcc SKILL。

  使用方式：用户提供法律分析报告或风险描述文本即可，SKILL 自动完成五步抽取与四图输出。
---
