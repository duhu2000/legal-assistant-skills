...
**担保资信评级：** {A / B / C / D} 级 · {可接受 / 附条件接受 / 不建议接受} · {一句话结论}

---

## 执行摘要 · 决策摘要

> **一句话结论：** {担保方是谁、净资产与担保占用底盘、有无致命司法风险、关联担保与否、给什么评级、建议担保金额上限}
...
**建议担保金额上限：** {净资产的 X%（按评级档位口径）/ 不计入风险缓释}

**建议行动（按紧迫度）：** 1. [T+0] … 2. [T+3] … 3. [T+7] …

---

## 1 核查结论 · 决策摘要

{评级 + 剩余可用担保额度口径说明 + 关键风险 + 建议担保金额上限 + 增信措施要点，3-5 句业务语言}
...
{按评级档位：A → 净资产 50% / B → 30%（附反担保或保证金）/ C → 10%（需换人或补充抵押）/ D → 不计入风险缓释。仅引用评级档位口径，不代客算出具体额度数字}

### 7.3 增信措施

{A → 标准连带责任保证合同；B → 反担保 + 实控人个人连带 + 交叉违约；C → 追加土地抵押 / 应收账款质押 + 季度报送财报；D → 放弃该担保方、重选主体}
